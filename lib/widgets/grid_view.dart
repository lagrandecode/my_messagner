...
class _GridViewBuilderState extends State<GridViewBuilder> {
  ...
  void _navigateIndex(){
    if(widget.slide.pages == 0){
      Navigator.of(context).push(MaterialPageRoute(builder: (_)=>Delivery(),),);
    }
    else
    if(widget.slide.pages == 1){
      Navigator.of(context).push(MaterialPageRoute(builder: (_)=>Courier(),),);
    }
    else
    if(widget.slide.pages == 2){
      Navigator.of(context).push(MaterialPageRoute(builder: (_)=>Food(),),);
    }
    else{
      Navigator.of(context).push(MaterialPageRoute(builder: (_)=>Cleaning(),),);
    }
      ...
      child: Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            Text(widget.slide.name, style: TextStyle(fontWeight: FontWeight.bold, color: widget.slide.txtColor), textAlign: TextAlign.center,)
          ],
        )
      ),
    );
